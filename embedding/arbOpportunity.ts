...
async function embedCleanArbOpportunity() {
  try {
    await cleanedClient.connect();
    console.log("Connected to Cleaned DB ✅");

    // Fetch rows from CleanArbOpportunity
    const res = await cleanedClient.query("SELECT * FROM CleanArbOpportunity");
    const rows = res.rows;

    // Create pgvector index if needed
    try {
      await pgVector.createIndex({
        indexName: "arb_opportunity_embedding_index",
    ...
    for (const row of rows) {
      ...
      try {
        ...
        // Insert into EmbeddedArbOpportunity table
        await cleanedClient.query(
          `INSERT INTO EmbeddedArbOpportunity (opportunity_id, text, embedding, metadata)
           VALUES ($1, $2, $3, $4)`,
          [
            row.id,
            text,
        ...
        console.log(`✅ Embedded & saved opportunity id ${row.id}`);
      } catch (err) {
        console.error(`❌ Embedding failed for opportunity id ${row.id}:`, err);
      }
    }
  } catch (err) {
...
// Run it
embedCleanArbOpportunity().catch(console.error);
