...
const TABLE_NAME = 'BTCoinAfterEight';

export async function insertBTCoinAfterEight() {
  await connectDB();
  await connectCleanedDB();

  await cleanedClient.query(`
    CREATE UNIQUE INDEX IF NOT EXISTS "idx_btcoinaftereight_row_hash"
    ON "BTCoinAfterEight" ("row_hash");
  `);
  ...
  const result = await client.query(
    `SELECT * FROM "BTCoinAfterEight" WHERE "id" > $1 ORDER BY "id" ASC`,
    [lastProcessedId]
  );
  ...
  const allColumns = [
    "id", "tokenAddress", "oldDate", "oldCoinPrice", "newDate", "newCoinPrice"
  ];
  ...
  for (let i = 0; i < hashedRows.length; i += batchSize) {
    ...
    await cleanedClient.query(`
      INSERT INTO "BTCoinAfterEight" (${insertColumns.map(c => `"${c}"`).join(', ')})
      VALUES ${values.join(', ')}
  ...
  await client.end();
  await cleanedClient.end();
  console.log(`✅ BTCoinAfterEight: ${hashedRows.length} rows inserted. Last ID: ${maxId}`);
}
...
insertBTCoinAfterEight().catch(console.error);
